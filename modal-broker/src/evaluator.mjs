...
export function fixtureFor(lessonSlug, code) {
  switch (lessonSlug) {
    ...
    case 'linux-read-text':
      return linuxFixture(code, "printf 'one\\ntwo\\nthree\\nfour\\nfive\\n' > /workspace/app.log", "head -n 2 /workspace/app.log > /workspace/expected_head; tail -n 2 /workspace/app.log > /workspace/expected_tail; grep -Fq 'one' /workspace/learner.out; grep -Fq 'two' /workspace/learner.out; grep -Fq 'four' /workspace/learner.out; grep -Fq 'five' /workspace/learner.out; grep -Eq '(^|[^0-9])5([^0-9]|$)' /workspace/learner.out")
    ...
    case 'linux-grep-find':
      return linuxFixture(code, "mkdir -p /workspace/config/nested; printf 'enabled=true\\n' > /workspace/config/app.conf; printf 'enabled=false\\n' > /workspace/config/off.conf; printf 'enabled=true\\n' > /workspace/config/nested/extra.conf; printf 'enabled=true\\n' > /workspace/config/ignore.txt", "grep -Fq '/workspace/config/app.conf' /workspace/learner.out || grep -Fq 'config/app.conf' /workspace/learner.out; grep -Fq 'extra.conf' /workspace/learner.out; ! grep -Fq 'off.conf' /workspace/learner.out; ! grep -Fq 'ignore.txt' /workspace/learner.out")
    ...
    case 'linux-environment-path':
      return linuxFixture(code, "mkdir -p /workspace/home/pathway/bin; printf '#!/bin/sh\\nprintf \\\"hello\\\\n\\\"\\n' > /workspace/home/pathway/bin/hello-pathway; chmod +x /workspace/home/pathway/bin/hello-pathway", "grep -Fxq '/workspace/home/pathway/bin/hello-pathway' /workspace/learner.out; grep -Eq '^/workspace/home/pathway/bin:' <(env PATH=\"$PATH\" printf '%s\\n' \"$PATH\") || true")
...
; grep -Fxq 'Hello, Pathway!' /workspace/learner.out")
    ...
    case 'bash-safe-scripting':
      return linuxFixture(code, '', "test -x /workspace/safe-temp.sh; grep -Fq 'Pathway' /workspace/learner.out; grep -Eq 'set[[:space:]]+-euo[[:space:]]+pipefail' /workspace/safe-temp.sh; grep -Fq 'mktemp' /workspace/safe-temp.sh; grep -Fq 'trap' /workspace/safe-temp.sh")
